DIST_INNOV：ViT→HRNet 渐进剪枝蒸馏（动态权重 + 姿态感知通道保护）

本目录面向“ViT 教师 + HRNet 学生”的人体姿态估计蒸馏与结构化剪枝联合训练，提供两项可独立开关的创新点实现，以及完整消融实验配置。

---

## 摘要

我们提出一套面向姿态估计“跨架构蒸馏 + 渐进式结构化剪枝”的统一训练框架，以 ViT 教师为知识源、HRNet 学生为高分辨率定位骨干。在训练过程中，学生网络按固定周期进行通道级结构化剪枝，累计剪枝率逐步提升并伴随恢复阶段。针对固定蒸馏权重与任务无关的通道重要性等问题，我们引入两项互补改进：（1）剪枝率与 epoch 双维度绑定的动态蒸馏权重调度；（2）教师姿态原型引导的关节-通道贡献标定与强通道保护（含长尾关节配额提升）。该实现提供完整消融配置与可复现的训练脚本。
...
该实现满足“任务驱动的通道重要性评估”的核心要求：通道打分的空间权重直接来自教师的 2D 关节热图响应（并可被教师 proto 能量调制），而非权重范数/特征图秩等间接指标。
...
- proto 蒸馏使用 `1x1` 通道投影适配器（`proto_adaptor_out_channels`），teacher proto 支持可选归一化（`normalize_teacher_proto`）。

### 分支差异化剪枝上限（Branch-wise Max-Ratio Clamp）

为避免“统一比例剪枝”误伤高分辨率分支，本实现支持对 HRNet 不同 stage/branch 设置最大可剪枝率上限：当搜索到的 `extra.num_channels` 低于该上限对应的最小保留通道数时，会被自动 clamp 回安全范围。工程入口为 `HRNetPruneRecoverHook._apply_branch_max_prune_ratio(...)`，配置项为：

- `prune_hook.max_prune_ratio_stage3={branch_id: max_ratio, ...}`
- `prune_hook.max_prune_ratio_stage4={branch_id: max_ratio, ...}`

默认配置已按“高分辨率分支更保守、低分辨率分支更激进”的思路给出一组上限：高分辨率分支上限约 0.112，低分辨率分支上限约 0.743，用于对齐“多分辨率异构冗余”的先验。

### 多分辨率热图监督（Multi-Resolution Heatmap Supervision）

为把“任务监督”直接下沉到 HRNet 多分支特征上，本实现支持在 stage3/stage4 指定分支挂载轻量 `1x1 conv` 辅助头，直接预测 heatmap 并与监督目标计算损失。该机制与姿态感知剪枝互补：一方面为多分辨率分支提供显式任务约束，另一方面也能在高剪枝率阶段稳定梯度与收敛。

- 配置项：`distill_cfg.heatmap_supervision`
  - `enable`：是否启用
  - `target`：`gt|teacher|mix`（默认 `gt`）
  - `loss`：`mse|kl`（默认 `mse`）
  - `branches`：形如 `(('stage3',2,w),('stage4',2,w),...)` 的分支列表与权重
- 工程特性：剪枝重建学生网络后会自动重建辅助头，并尽可能做权重切片继承以提升稳定性
...
- `distill_cfg.heatmap_supervision`：`enable/target/loss/branches`
- `prune_hook`（`HRNetPruneRecoverHook`）：
  ...
  - `branch_weight_stage3/branch_weight_stage4`：剪枝率→通道数搜索时的分支缩放系数（用于体现“多分辨率冗余异构”，默认已给出一组保守权重）

## 相关工作与开源（用于叙述与复现对照）

- HRNet 轻量化与多分辨率冗余：Lite-HRNet（CVPR 2021，官方实现与模型）https://github.com/HRNet/Lite-HRNet ，论文页 https://arxiv.org/abs/2104.06403
- HRNet 高分辨率分支在不同算力区间的“可删/不可删”现象：LitePose（CVPR 2022，代码与论文）https://github.com/mit-han-lab/litepose ，论文页 https://openaccess.thecvf.com/content/CVPR2022/html/Wang_Lite_Pose_Efficient_Architecture_Design_for_2D_Human_Pose_Estimation_CVPR_2022_paper.html
- “结构变化过程中蒸馏目标应随之匹配”的动态蒸馏思路：Joint-DetNAS（CVPR 2021）https://openaccess.thecvf.com/content/CVPR2021/html/Yao_Joint-DetNAS_Upgrade_Your_Detector_With_NAS_Pruning_and_Dynamic_Distillation_CVPR_2021_paper.html
- 渐进式缩减/多阶段训练范式（与本项目的 step_schedule + recovery 对齐）：Once-for-All（ICLR 2020）https://iclr.cc/virtual_2020/poster_HylxE1HKwS.html ，代码 https://github.com/mit-han-lab/once-for-all
- 通用结构化剪枝工程（依赖图/剪枝可达性）：Torch-Pruning / DepGraph（CVPR 2023）https://github.com/VainF/Torch-Pruning
...
- 默认配置（动态权重 + 姿态剪枝 + 强保护 + 热图辅助对齐诊断）：[full_innov_3methods.py](file:///root/rivermind-data/PoseBH/experiments/DIST_INNOV/full_innov_3methods.py)
